...
>
    ...
    <div
        ...
        x-bind:class="{
            'sm:w-fit sm:flex-none sm:flex-col sm:items-stretch': vertical
        }"
        class="flex items-center font-sf text-lg text-sm dark:border-zinc-700 overflow-x-auto md:overflow-x-visible"
    >
        @foreach($tabs as $key => $tab)
            <button
                ...
                x-bind:class="{
                    'text-zinc-950 dark:text-zinc-50 border-secondary dark:border-zinc-700/75 bg-white dark:bg-zinc-900': active === '{{ $key }}',
                ...
                class="z-10 -mb-px flex items-center cursor-pointer gap-2 rounded-t-lg border-x border-t text-xl px-10 py-3 font-medium"
            >
                {{ $tab['title'] }}
            </button>
        @endforeach
    ...
    <!-- Tab Content -->
    <div class="w-full rounded-b-lg rounded-tr-lg border border-secondary bg-white p-5 dark:border-zinc-700/75 dark:bg-zinc-900">
        @foreach($tabs as $key => $tab)
            <div
                x-coak
                x-show="active === '{{ $key }}'"
