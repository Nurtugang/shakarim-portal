...
class WelcomeBlock extends TiptapBlock
{
    public string $preview = 'blocks.previews.welcome';

    public string $rendered = 'blocks.rendered.welcome';
    public ?string $label = 'welcome';
    ...
    ->columns(2),
    Fieldset::make('Кнопка 2')
    ->schema([
        TextInput::make('second_button_label')
            ->label('Label '),
        TextInput::make('second_button_link')
            ->label('Ссылка'),
        TextInput::make('second_button_icon')
            ->label('Иконка'),
        Radio::make('second_button_class')
            ->options([
        'btn-primary' => 'Primary',
        'btn-secondary' => 'Secondary',
    ])->label('Стиль')
